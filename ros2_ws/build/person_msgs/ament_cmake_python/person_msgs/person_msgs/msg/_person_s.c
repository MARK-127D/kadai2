...
bool person_msgs__msg__person__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // age1
    PyObject * field = PyObject_GetAttrString(_pymsg, "age1");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->age1 = (uint8_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
...
PyObject * person_msgs__msg__person__convert_to_py(void * raw_ros_message)
{
  ...
  {  // age1
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->age1);
    {
      int rc = PyObject_SetAttrString(_pymessage, "age1", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
